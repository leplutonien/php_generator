...
class generate {
    ...
    private static function appConfiguration($base){
        ...
        foreach($routes as $route){
            ...
            if(!isset($actions[$route['module']])){
                ...
                $returnings[$route['module']] = array($route['action'] => isset($route['returning'])? $route['returning']: NULL) ;
            }else{
                ...
                if(!in_array( $route['action'],$elts)){
                    ...
                    $returnings[$route['module']][$route['action']] = isset($route['returning'])? $route['returning']: NULL ;
                }
            }
        }
        ...
        if(count($actions) > 0) {
            foreach ($actions as $module => $action) {
                ...
                foreach ($elts as $elt) {
                    ...
                    $r = self::getReturning($returnings,$module,$elt);
                    ...
                    if(is_null($r)) {
                        ...
                        $fcts .= "\n\tpublic function execute" . ucfirst(BoxFunctions::getCustomizeName($elt)) . "(){"
                            ...
                            . "\n\t\t\$status = 0 ;"
                            . "\n\n"
                            . "\n\t\t\$this->page->addVar('response',BoxFunctions::ApiResponse(\$response,\$message,\$status));"
                            . "\n\t\t\$this->app->httpResponse()->addHeader('Content-Type: application/json');"
                            . "\n\t\t\$this->page->setLayout('default-layout.php');"
                            . "\n\t}\n";
                    ...
                    if(is_null($r))
                        ...
                        $view->setContent("<?php echo \$response ;?>");
                ...
                if( count($returnings) > 0 )
                    $file->setKey("dependencies", "use lib\BoxFunctions;");
                else
                    $file->setKey("dependencies", "");
    ...
    private static function init($base,$update){
        ...
        if(!$update){
            ...
            if(self::$json->getAttribute('sessionToken')){
                mkdir($base.'/additionals/session_token');
                copy(dirname(__FILE__).'/additionals/session_token/JWT.php',$base.'/additionals/session_token/JWT.php');
                copy(dirname(__FILE__).'/additionals/session_token/SessionToken.php',$base.'/additionals/session_token/SessionToken.php');
            }
    ...
    private static function generateEntities($tables,$base){
        foreach($tables as $table){
            ...
            $entityFile->setKey('_delete', "return self::$".lcfirst($entityName)."Dao->remove(\$this);");
            $entityFile->setKey('delete', "return self::$".lcfirst($entityName)."Dao->delete(\$conds);");
    ...
    private static function generateModels($tables, $base){
        foreach($tables as $table){
            ...
            $modelFile->setKey('dparams', $dparams);
    ...
    /**
     * Get a returning of an action in a route
     * @param array $returnings
     * @param $module
     * @param $action
     * @return null
     */
    private static function getReturning(array $returnings, $module, $action){

        foreach($returnings as $mod => $actArray){
            if($mod == $module){
                foreach($returnings[$module] as $act => $returning){
                    if($act ==  $action){
                        return $returning;
                    }
                }
